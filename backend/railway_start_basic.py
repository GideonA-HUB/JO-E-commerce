#!/usr/bin/env python
"""
Basic Railway startup script for TASTY FINGERS
Uses Railway's default port and minimal configuration
"""
import os
import sys
...
def start_server():
    """Start the Gunicorn server with basic configuration"""
    ...
    # Start Gunicorn with basic configuration - let Railway handle the port
    cmd = "gunicorn backend.wsgi --bind 0.0.0.0:$PORT --workers 1 --timeout 120"
    print(f"Running: {cmd}")
    os.system(cmd)

if __name__ == '__main__':
    print("🚀 Starting TASTY FINGERS on Railway (Basic Mode)...")
    print("=" * 60)
